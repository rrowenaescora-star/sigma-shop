...
useEffect(() => {
  ...
  async function loadProducts() {
    try {
      ...
      const response = await fetch("/api/products?game=anime-defenders", { cache: "no-store" });
     ...
     const animeDefendersProducts = (result.products || []).filter(
  (product: Product) => product.game === "anime-defenders"
);

setProducts(animeDefendersProducts);

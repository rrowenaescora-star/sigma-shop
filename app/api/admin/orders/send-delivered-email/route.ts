...
export async function POST(req: Request) {
  try {
            ...
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td width="62" valign="middle">
                  <img src="${itemImage}" alt="${itemName}" width="52" height="52" style="display:block; border-radius:10px; object-fit:cover; background:#0f172a; border:0;" />
                </td>
    ...
    console.error("Send delivered email error:", error);

    return NextResponse.json(
      { error: "Server error while sending delivered email." },
      { status: 500 }
    );
  }
}
...
export async function GET() {
  ...
  const events = (data || []).filter((event) => !event.customer_email || !adminEmails.includes(event.customer_email.toLowerCase()));
  ...
  const summarise = (key: "path" | "country" | "device", items = events) => Object.entries(items.reduce<Record<string, number>>((total, event) => { const label = key === "country" ? countryName(event.country) : event[key] || "Unknown"; total[label] = (total[label] || 0) + 1; return total; }, {})).sort((a, b) => b[1] - a[1]).slice(0, 8).map(([label, value]) => ({ label, value }));
  return NextResponse.json({
    ...
    pages: summarise("path"), countries: summarise("country"), devices: summarise("device"), recent: events.slice(0, 40).map((event) => ({ ...event, country: countryName(event.country) })),
  });
}
...
useEffect(() => {
  ...
  async function loadProducts() {
    try {
      ...
      const response = await fetch("/api/products?game=pet-simulator", { cache: "no-store" });
     ...
     const petSimulatorProducts = (result.products || []).filter(
  (product: Product) => product.game === "pet-simulator"
);

setProducts(petSimulatorProducts);

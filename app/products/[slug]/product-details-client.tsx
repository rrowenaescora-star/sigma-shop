...
export default function ProductDetailsPage({ initialProduct }: { initialProduct: Product | null }) {
  ...
  useEffect(() => {
    ...
    const updatedIds = [product.id, ...priorIds.filter((id) => id !== product.id)].slice(0, 12);
    localStorage.setItem("recently-viewed-products", JSON.stringify(updatedIds));
    const recentIds = updatedIds.filter((id) => id !== product.id);
    if (recentIds.length === 0) return;
    fetch(`/api/products?ids=${recentIds.join(",")}`, { cache: "no-store" })
      .then((response) => response.json())
      .then((data) => {
        const products: Product[] = data.products || [];
        setRecentProducts(recentIds.map((id) => products.find((item) => item.id === id)).filter((item): item is Product => Boolean(item)));
      })
      .catch(() => {});
  }, [product]);
  ...
  return (
    <>
      <header className="product-header-enter fixed inset-x-0 top-0 z-50 border-b border-blue-500/10 bg-[#07111f]/95 shadow-[0_15px_50px_rgba(0,0,0,0.45)] backdrop-blur-xl">
        <div className="relative overflow-hidden">
          <div className="absolute inset-0 hidden md:block">
            <img src="/blof-fruit-logo.webp" alt="" className="h-full w-full object-cover opacity-45" />
          ...
          <div className="relative mx-auto flex max-w-[1850px] items-center justify-between px-5 py-3 md:px-8">
            <Link href="/" className="flex items-center gap-3">
              <img src="/logo.webp" alt="Bloxhop" className="h-11 w-11 object-contain" />
      ...
      <footer className="relative mt-14 -mx-4 overflow-hidden border-t border-white/10 sm:-mx-6 lg:-mx-10">
        <img src="/blox-fruit-logo.webp" alt="" className="h-[180px] w-full object-cover object-center opacity-75 md:h-[260px]" />

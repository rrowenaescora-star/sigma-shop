...
export default function StealAnEggOrderClient(){
  ...
  function exitCompletedOrder(){
    if(window.parent!==window){window.parent.postMessage({type:"steal-an-egg-order-completed-close"},window.location.origin);return;}
    window.location.assign("/steal-an-egg");
  }
  ...
  if(!paid)return <main className="flex min-h-screen items-center justify-center bg-[#07111f] px-5 text-white"><div className="max-w-lg rounded-3xl border border-red-400/20 bg-[#0b1628] p-8 text-center"><h1 className="text-3xl font-black">Instructions locked</h1><p className="mt-4 text-slate-300">We could not confirm a completed payment for this checkout session.</p><div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center"><Link href="/steal-an-egg" className="inline-flex justify-center rounded-xl bg-[#7eeb00] px-5 py-3 font-black text-black">Return to shop and claim order</Link><Link href="/steal-an-egg/checkout" className="inline-flex justify-center rounded-xl bg-cyan-500 px-5 py-3 font-bold">Return to checkout</Link></div></div></main>;
  ...
  const friendRequestSent=waiting||verified;
  const serverLinkReady=accountSelected&&Boolean(fulfillment.invitationUrl);
  const delivered=fulfillment.deliveryStatus==="Delivered";

  if(delivered)return <main className={embedded?"bg-transparent p-0 text-white":"flex min-h-screen items-center justify-center bg-[#031827] px-4 py-10 text-white"}>{!embedded&&<div className="pointer-events-none fixed inset-0 bg-[linear-gradient(135deg,rgba(0,181,216,.24),transparent_42%),radial-gradient(circle_at_bottom_right,rgba(126,235,0,.2),transparent_35%)]"/>}<section data-embedded-order-card className={`relative w-full overflow-hidden rounded-2xl border-[4px] border-black bg-gradient-to-b from-[#08bad0] via-[#087aa2] to-[#07345f] p-1 ${embedded?"max-w-none":"max-w-3xl shadow-[10px_10px_0_#000]"}`}> <div className="rounded-xl border-2 border-black bg-[#061b2f]/90 p-7 text-center sm:p-12"><img src="/steal-an-egg-logo.png" alt="Steal an Egg" className="mx-auto h-24 w-auto max-w-full object-contain drop-shadow-[0_5px_0_rgba(0,0,0,.8)] sm:h-32"/><div className="mx-auto mt-6 flex h-20 w-20 items-center justify-center rounded-full border-[3px] border-black bg-[#7eeb00] text-4xl font-black text-black shadow-[4px_4px_0_#000]">✓</div><p className="mt-6 inline-flex rounded-md border-2 border-black bg-[#7eeb00] px-4 py-1 text-xs font-black uppercase tracking-[.25em] text-black">Order completed</p><h1 className="mt-5 text-4xl font-black text-white [text-shadow:3px_3px_0_#000] sm:text-5xl">THANK YOU!</h1><p className="mx-auto mt-4 max-w-lg text-base font-semibold leading-7 text-cyan-50">Your Steal an Egg item has been delivered successfully. Thank you for choosing Bloxhop Online Store!</p><div className="mx-auto mt-6 max-w-sm rounded-xl border-[3px] border-black bg-[#f4fbff] px-5 py-4 text-[#07111f] shadow-[5px_5px_0_#000]"><p className="text-sm font-bold text-slate-600">ORDER #{orderId}</p><p className="mt-2 text-xl font-black text-[#087397]">✓ DELIVERY COMPLETE</p></div><button onClick={exitCompletedOrder} className="mt-8 inline-flex min-h-14 w-full max-w-sm items-center justify-center rounded-lg border-[3px] border-black bg-[#7eeb00] px-6 py-3 text-lg font-black text-black shadow-[5px_5px_0_#000] transition hover:-translate-y-0.5 hover:bg-[#92ff14]">BACK TO MERCHANT</button></div></section></main>;  return <main className={embedded?"bg-transparent p-0 text-white":"min-h-screen bg-[#031827] px-4 py-8 text-white sm:px-6"}>{!embedded&&<div className="pointer-events-none fixed inset-0 bg-[linear-gradient(135deg,rgba(0,181,216,.2),transparent_42%),radial-gradient(circle_at_bottom_right,rgba(126,235,0,.12),transparent_35%)]"/>}<div data-embedded-order-card className={`relative overflow-hidden rounded-2xl border-[4px] border-black bg-[#061b2f] shadow-[9px_9px_0_#000] ${embedded?"w-full max-w-none shadow-none":"mx-auto max-w-5xl"}`}> 
    ...
    <div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[1.15fr_.85fr]">
      <section className="space-y-6">
        ...
        <div className={`rounded-xl border-[3px] border-black p-5 shadow-[4px_4px_0_#000] ${serverLinkReady?"border-emerald-400/30 bg-emerald-400/5":"border-white/10 bg-white/[.03]"}`}><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-md border-2 border-black bg-[#7eeb00] font-black text-black">3</span><h2 className="text-xl font-black">Join the delivery server</h2></div><p className="mt-3 text-sm leading-6 text-slate-300">{delivered?"Your item delivery is complete.":serverLinkReady?"Your server link is ready. Open Roblox and join the server so BloxhopOnlineStore can deliver your item.":accountSelected?"Your account is selected. The server link will appear here automatically as soon as Bloxhop staff saves it.":"Select your Roblox account in Step 1 to unlock the server link."}</p>{serverLinkReady&&<a href={fulfillment.invitationUrl!} target="_blank" rel="noreferrer" className="mt-4 flex w-full items-center justify-center rounded-lg border-[3px] border-black bg-[#7eeb00] px-5 py-3 font-black text-black shadow-[4px_4px_0_#000] hover:bg-[#92ff14]">Join Delivery Server ↗</a>}{delivered&&<p className="mt-4 font-black text-emerald-300">✓ Delivery completed</p>}</div>
        {message&&<p className="rounded-xl border border-red-400/20 bg-red-400/10 p-4 text-sm text-red-200">{message}</p>}
      </section>

      <aside><h2 className="text-lg font-black">Live status</h2><div className="mt-4 space-y-3">{[[true,"Payment confirmed"],[accountSelected,"Roblox account selected"],[friendRequestSent,"Friend request submitted"],[serverLinkReady||delivered,"Server link ready"],[delivered,"Delivery completed"]].map(([done,label],index)=><div key={String(label)} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[.03] p-3"><span className={`flex h-7 w-7 items-center justify-center rounded-full text-sm font-black ${done?"bg-emerald-500 text-white":"bg-slate-700 text-slate-400"}`}>{done?"✓":index+1}</span><span className={done?"font-bold text-white":"text-sm text-slate-400"}>{String(label)}</span></div>)}</div><p className="mt-5 text-xs leading-5 text-slate-500">Status refreshes automatically every five seconds. You can safely keep or revisit this page in the same browser.</p></aside>
    </div>
  </div></main>;
}
...
const paidDate = (value: string) =>
  new Intl.DateTimeFormat("en-US", { dateStyle: "long", timeStyle: "short" }).format(new Date(value));

const getItems = (items: unknown): LineItem[] =>
  Array.isArray(items)
    ? items.map((item) => ({
...
export function buildInvoiceEmail(order: InvoiceOrder) {
...
export async function buildInvoicePdf(order: InvoiceOrder): Promise<Buffer> {
  const document = new PDFDocument({ size: "A4", margin: 0, info: { Title: `Bloxhop Invoice BLOX-${order.id}`, Author: "Bloxhop Online Store" } });
  const chunks: Buffer[] = [];
